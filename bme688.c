...
volatile int ventana_tiempo = 5000;      // en milisegundos
...
float leer_humedad(void) {
    ...
    // Sumar un 40% adicional (equivalente a 40% de humedad)
    hum_comp += (int32_t)(0.40 * 1024 * 100); // 40% en la escala usada

    // Ajustar la humedad nuevamente al rango 0% - 100% después del ajuste
    if (hum_comp > 100000) {
        hum_comp = 100000;
    } else if (hum_comp < 0) {
        hum_comp = 0;
...
static void event_handler(void *arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        esp_wifi_connect();
        ESP_LOGI(TAG, "Intentando reconectar al AP");
...
void tcp_client_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // Aquí puedes agregar código para enviar/recibir datos
...
void app_main(void) {
    ...
    // Inicializar WiFi y conectar al servidor
    wifi_init_sta();
